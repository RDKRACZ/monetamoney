...
public class AuctionHomePage extends AbstractContainerScreen<AuctionHomeContainer> {
    ...
    protected void renderBg(PoseStack matrixStack, float partialTicks, int x, int y) {
        ...
        blit(matrixStack, this.getBackgroundXStart(), this.getBackgroundYStart(), 0, 0, 222, 256, 256, 302);
    }
    ...
    protected void slotClicked(Slot slotIn, int slotId, int mouseButton, ClickType type) {
        if (slotIn != null && type == ClickType.PICKUP && slotIn.index > 35) {
            if (!slotIn.getItem().isEmpty()) {
                PacketHandler.CHANNEL.sendToServer(new COpenBuyPage(slotIn.getItem().save(new CompoundTag())));
            }
    ...
    private void toggleButton(Button pressedButton) {
        if (this.editMode) {
            this.renderables.remove(pressedButton);
