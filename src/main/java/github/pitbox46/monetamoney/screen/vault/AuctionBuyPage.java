...
public class AuctionBuyPage extends AbstractContainerScreen<AuctionBuyContainer> implements IStatusable {
    ...
    protected void init() {
        ...
        if (!editMode) {
            this.addRenderableWidget(new ImageTextButton(this.getBackgroundXStart() + 62, this.getBackgroundYStart() + 84, 100, 23, 0, 217, 23, TEXTURE, 256, 263, button -> {
                PacketHandler.CHANNEL.sendToServer(new CTransactionButton(this.menu.handler.getStackInSlot(0).getOrCreateTag().getInt("price"), CTransactionButton.Button.PURCHASE));
    ...
    protected void renderBg(PoseStack matrixStack, float partialTicks, int x, int y) {
        ...
        blit(matrixStack, this.getBackgroundXStart(), this.getBackgroundYStart(), 0, 0, 222, 217, 256, 263);
    }
    ...
    protected void drawInfoStrings(PoseStack matrixStack) {
        if (!this.menu.handler.getStackInSlot(0).isEmpty()) {
            ...
            drawCenteredString(matrixStack, this.font, new TranslatableComponent("info.monetamoney.price", this.menu.handler.getStackInSlot(0).getTag().getInt("price")), width / 2, this.getBackgroundYStart() + 55, FastColor.ARGB32.color(255, 255, 255, 255));
        }
